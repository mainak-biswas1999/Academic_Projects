...
class RNN(nn.Module):
  ...
  def forward(self, s1, s2):
    ...
    _, h_t = self.rnn_s1(s1_emb)    # a0)
    out_s1 = h_t[-1, :, :]  #reorder
...
class LSTM(nn.Module):
  #initializer
  #this contains input_layer size (length of the word embeddings: Here word2vec/embedding layers),
  def __init__(self, vocab_size, input_size, hidden_size, fc_size, num_layers, num_labels, dropout_rnn, dropout_fc, embedding_w = None, is_trainable=True):
    #construct the parent
    ...
    if embedding_w == None: 
      self.Emb_layer = nn.Embedding(vocab_size, input_size, padding_idx=0)
      self.Emb_layer.weight.requires_grad = is_trainable 
    
    else:
      self.Emb_layer = nn.Embedding(vocab_size, input_size, padding_idx=0)
      self.Emb_layer.weight = torch.nn.Parameter(embedding_w)
      self.Emb_layer.weight.requires_grad = is_trainable
  ...
  def forward(self, s1, s2):
    ...
    _, (h_t, c_t) = self.lstm_s1(s1_emb)    # a0)
    out_s1 = h_t[-1, :, :]  #reorders properly
    #storing only the output of the last timestamp
    #out_s1 = out_s1[:, -1, :]

    #output is of (batch_size, sentence_l, output) -> just need the last o/p (as a feature for the sentence)
    _, (h_t, c_t) = self.lstm_s1(s2_emb)   #a0)
    #out_s2 = out_s2[:, -1, :]
    out_s2 = h_t[-1, :, :]
...
class GRU(nn.Module):
  ...
  def __init__(self, vocab_size, input_size, hidden_size, fc_size, num_layers, num_labels, dropout_rnn, dropout_fc):
    ...
    #from the torch.nn module we initialize an rnn object (if multiple stacked RNNs are present num_layers is used)
    #batch first is done in accordance to keras convention of (None, sentence length, word_emb-size
    self.gru_s1 = nn.GRU(input_size=input_size, hidden_size=hidden_size, num_layers=num_layers, batch_first=True, dropout=dropout_rnn)
    self.gru_s2 = nn.GRU(input_size=input_size, hidden_size=hidden_size, num_layers=num_layers, batch_first=True, dropout=dropout_rnn)
    #concatenate outputs of network 1 and 2 and then define a fully connected layer followed by softmax classification
    #fully connected layer: (taking the input from both the recurrent unit)
    self.fully_connected_1 = nn.Linear(2*hidden_size, fc_size)
    self.fc_dropout = nn.Dropout(dropout_fc)
  ...
  def forward(self, s1, s2):
    ...
    _, h_t = self.gru_s1(s1_emb)    # a0)
    out_s1 = h_t[-1, :, :]  #reorders properly
    #storing only the output of the last timestamp
    #out_s1 = out_s1[:, -1, :]

    #output is of (batch_size, sentence_l, output) -> just need the last o/p (as a feature for the sentence)
    _, h_t = self.gru_s1(s2_emb)   #a0)
    #out_s2 = out_s2[:, -1, :]
    out_s2 = h_t[-1, :, :]
  ...
  def initialize_weights(self):
    for name, param in self.gru_s1.named_parameters():
      if 'bias' in name:
        nn.init.constant_(param, 0.0)
      elif 'weight_ih' in name:
        nn.init.xavier_normal_(param)
    ...
    for name, param in self.gru_s2.named_parameters():
      if 'bias' in name:
        nn.init.constant_(param, 0.0)
      elif 'weight_ih' in name:
        nn.init.xavier_normal_(param)
